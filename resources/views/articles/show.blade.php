...
@section('og_image'){{asset('storage/files/600x450/'.$article->files->first()->filename)}}@endsection
@endif
@section('url'){{action('ArticleController', [$article->id])}}@endsection
@endif
                        ...
                        <div class="user-article-block">
                            ...
                            <ul class="list-unstyled">
                                @foreach($userArticles as $userArticle)
                                    <li><a href="{{route('news.show', $userArticle->id)}}">{{$userArticle->title}}</a>
                                    </li>
                                @endforeach
                            </ul>
                        </div>
